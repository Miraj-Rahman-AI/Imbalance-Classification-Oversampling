...
from experiments_all import (  # Import custom functions for data loading, model evaluation, etc.
    load_2classes_data, load_kto2_classes_data_lm, get_classifier, get_metrics,
    load_k_classes_data, write_excel, now_time_str, sampling_methods, mean_std
)
...
def resamplings(data_train, methods):
    ...
    # Apply NaN_RD to handle missing data (commented out for now)
...
def run(result_file_path, data_names, classifiers, methods, noise_rate, is_binary: int, is_raise: bool = False):
    """
    ...
    for d, data_name in enumerate(data_names):
        ...
        elif is_binary == 0:  # Multi-class datasets
    ...
    df.to_csv(r'KBsomte/SMOTE/Log/' + str(noise_rate) + '.csv', index=False)


# Main function for binary classification
def main_binary(is_binary: int = 1, is_raise: bool = False):
    """
    ...
    methods = (
        'MWMOTE', 'AdaptiveSMOTE', 'SMOTE_TomekLinks', 'SMOTE_IPF', 'RSMOTE', 'kmeans-smote', 'PAO', 'SWIM',
    )
    ...
    run(result_file_path, data_names, classifiers, methods, noise_rate=0.3, is_binary=is_binary, is_raise=is_raise)
...
def main_multiClass(is_binary: bool = 0, is_raise: bool = False):
    ...
    data_names = [
        'wine', 'vertebralColumn', 'OBS', 'PhishingData', 'abalone', 'sensorReadings', 'frogs',
    ]
    ...
    # Define the result file path
    result_file_path = r'KBsomte/SMOTE/result table/multiClass/{} result multiClass.xls'.format(
        now_time_str(colon=False))
...
if __name__ == '__main__':
    # main_multiClass(is_binary=0)  # Run multi-class experiments
