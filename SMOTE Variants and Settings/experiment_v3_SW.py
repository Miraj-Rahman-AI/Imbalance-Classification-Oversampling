...
import _smote_variants_v3_SW,_imblearn_v3_SW,_smote_variants_v1_orignal
...
def plot_hyperplane(clf, X, y,
    ...
    for label in [0, 1]:
        plt.scatter(X[y == labels[label], 0], X[y == labels[label], 1], c=colors[label], s=6, cmap=plt.cm.RdYlBu)
...
counters = (samples, features, train0, train1, test0, test1)
    ...
    data_small_2_class = ['credit', 'german', 'heart1', 'hepatitis', 'horse', 'iono', 'wdbc']
    ...
    if data_name in uci_extended:
        df = pd.read_csv(experiment_path + r'/SMOTE/uci_extended/' + data_name + '.csv')
        data = df.values
        data = np.hstack((data[:, -1:], data[:, :-1]))
    elif data_name in data_gongye :             
        df = pd.read_csv(experiment_path + r'/data_set/digital_twin/' + data_name + '.csv')
        data = df.values
    elif data_name in data_big:
        df = pd.read_csv(experiment_path + r'/data_set/large_data/' + data_name + '.csv', header=None)
        data = df.values
    elif data_name in data_small:
        df = pd.read_csv(experiment_path + r'/data_set/small_data/' + data_name + '.csv', header=None)
        data = df.values
    # KEEL imb_IRlowerThan9
        ...
        data_mat = scipy.io.loadmat(experiment_path + r'/data_set/dataset16/dataset16.mat')
        if data_name == 'madelon':
            data = np.vstack((data_mat['madelon_train'], data_mat['madelon_test']))
    ...
    if imbalance_rate != 0 and data_name not in uci_extended:
        assert imbalance_rate > 1
        data_less_num = int(data_more.shape[0] // imbalance_rate)
        data_less = random_sampling(data_less, data_less_num, random_state=random_state)


    data_more_train, data_more_test = split_data_set(data_more, proportion, random_state=random_state)
    data_less_train, data_less_test = split_data_set(data_less, proportion, random_state=random_state)
    ...
    if add_noise_way == 'separately':
        ...
        add_noise(data_less_train, noise_rate=noise_rate, random_state=random_state)
...
def load_k_classes_data(data_name, proportion=(0.8, 0.2), noise_rate=0.0, normalized=True, random_state=0, n_rows=0):
    ...
    if data_name in data_small:
        df = pd.read_csv(r'/home/KBsomte/data_set/small_data/' + data_name + '.csv', header=None)
        data = df.values
    elif data_name in data_zy:
        df = pd.read_csv(r'/home/data_set/DataSet/' + data_name + '.csv', header=None)
        data = df.values
    else:
        assert 0 == 1, 'data set \'{}\' is not exist!'.format(data_name)
    ...
    labels = list(set(data[:, 0]))
    for label in labels:
        train, test = split_data_set(data[data[:, 0] == label, :], proportion, random_state=random_state)
        data_train = np.vstack((data_train, train))
        data_test = np.vstack((data_test, test))

    data_train = add_noise_lx(data_train, noise_rate=noise_rate, random_state=random_state)
...
def get_classifier(classifier, random_state=0):
    from sklearn.neural_network import MLPClassifier  # todo
    ...
    from xgboost import XGBClassifier  # todo
    from lightgbm import LGBMClassifier  # todo
    ...
    if classifier == 'BPNN':
        ...
        clf = SVC(random_state=random_state, probability=True)  # kernel='rbf', gamma='auto',
...
def sampling_methods(data_train=None, RData=None,method=None, parameters=(30, 5), random_state=42,weight=None,ntree=None):
    """
...
#Pre-process-------------------------------------------------------------------------
    if method == 'smote':
        ...
        model = over_sampling.BorderlineSMOTE(kind='borderline-1', random_state=random_state)
    elif method == 'ADASYN':
        model = _smote_variants_v1_orignal.ADASYN(random_state=random_state)
    elif method == 'kmeans-smote':
        model = over_sampling.KMeansSMOTE(random_state=random_state,)
    elif method =='svm-smote':
        model = over_sampling.SVMSMOTE(random_state=random_state)


    elif method == 'smote-SW':
        model = _imblearn_v3_SW.SMOTE(random_state=random_state,weight=weight,ntree=ntree)
    elif method == 'boderline1-smote-SW':
        model = _imblearn_v3_SW.BorderlineSMOTE(random_state=random_state,weight=weight,ntree=ntree)
    elif method == 'kmeans-smote-SW':
        model = _imblearn_v3_SW.KMeansSMOTE(random_state=random_state,weight=weight,ntree=ntree)
    elif method == 'ADASYN-SW':
        model = _smote_variants_v3_SW.ADASYN(random_state=random_state)
    elif method =='svm-smote-SW':
        model = _imblearn_v3_SW.SVMSMOTE(random_state=random_state,weight=weight,ntree=ntree)
#------------------------------------------------------------------------
...
#Post-process------------------------------------------------------------------------
        ...
        model = _smote_variants_v3_SW.SMOTE_TomekLinks(random_state=random_state)
...
#------------------------------------------------------------------------------------
        ...
        raise Exception(r'the sampling method name \'{}\' is not exist!'.format(method))

    if ('SMOTE_' in method) or ('ADASYN' in method): 
        if 'SW' not in method:      
            X_resampled, y_resampled = model.sample(data_train[:, 1:], data_train[:, 0])
        else:       
            X_resampled, y_resampled = model.sample(RData[:, 1:], RData[:, 0],weight,ntree)     
    else:   
        if "SW" not in method:      
            X_resampled, y_resampled = model.fit_resample(data_train[:, 1:], data_train[:, 0])
        else:       
            X_resampled, y_resampled = model.fit_resample(RData[:, 1:], RData[:, 0])
...
def __test_sampling():
    ...
    methods = ('smote', 'borderline1_smote', 'borderline2_smote', 'svm_smote', 'adasyn', 'kmeans_smote')
    ...
    for data_name in data_names:
        ...
        print('{:20s}：'.format(''), [counter[k] for k in range(len(counter))])

        for method in methods:
            data_train_resampled = sampling_methods(data, method)
            counter2 = Counter(data_train_resampled[:, 0])

            print('{:20s}：'.format(method), [counter2[k] for k in range(len(counter2))])


def __test_fourclass_plot():
    data_name = 'fourclass'
    noise_rate = 0.0
    data_train, data_test = load_k_classes_data(data_name, noise_rate=noise_rate)
