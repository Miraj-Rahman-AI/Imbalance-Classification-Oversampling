...
import _smote_variants_v7_GBsmote
import _smote_variants_v2_Wsmote    # W-smotes
import _smote_variants_v3_SW    # SW
...
from MC_CCR import MultiClassCCR
from MC_RBO import MultiClassRBO
from mahakil import MAHAKIL
from PA import PAO
# from multi_imbalance.resampling.mdo import MDO
from SWIM import SwimMaha
...
from sklearn.metrics import (accuracy_score, recall_score, roc_auc_score,precision_recall_fscore_support,f1_score, roc_curve, precision_score, confusion_matrix)
import os
import subprocess
...
from imblearn.metrics import geometric_mean_score   #Gmean
...
def load_kto2_classes_data_lm(data_name, proportion=(0.8, 0.2), noise_rate=0.0, normalized=True, n_rows=0,random_state=0, add_noise_way='together'):
    data_small = ['anneal', 'credit', 'german', 'heart1', 'heart2', 'hepatitis', 'horse', 'iono', 'sonar', 'wdbc',
...
def load_k_classes_data(data_name, proportion=(0.8, 0.2), noise_rate=0.0, normalized=True, random_state=0, n_rows=0):
    ...
    if data_name in data_small:
        ...
        df = pd.read_csv(experiment_path +
                        r'data_set/DataSet/' + data_name + '.csv', header=None)
        data = df.values
    else:raise('data set \'{}\' is not exist!'.format(data_name))
...
def get_metrics(clf, data_train, data_test)->dict:
    ...
    # multi class
        ...
        auc = roc_auc_score(data_test[:, 0],predict_proba,multi_class='ovr',average='macro') 
...
def sampling_methods(data_train=None, RData=None,method=None, parameters=(30, 5), random_state=42, weight_WNND=None,weight_SW=None,ntree=None,nans=None,k_neighbors=5):
    """
...
:return: return the sampled result
    ...
    # original
        ...
        model = _smote_variants_v1_orignal.SMOTE(random_state=random_state)
    elif method == 'MSYN_':
    ...
    # WNND
        ...
        model = _smote_variants_v5_NanRd_v2.SMOTE(random_state=random_state)

    # Gsmote
    elif method == 'SMOTE_ENN-G':
        model = _smote_variants_v6_Gsmote.SMOTE_ENN(random_state=random_state)
    elif method == 'SMOTE_TomekLinks-G':
        model = _smote_variants_v6_Gsmote.SMOTE_TomekLinks(random_state=random_state)
    elif method == 'SMOTE_RSB-G':
        model = _smote_variants_v6_Gsmote.SMOTE_RSB(random_state=random_state)
    elif method == 'SMOTE_IPF-G':
        model = _smote_variants_v6_Gsmote.SMOTE_IPF(random_state=random_state)
    elif method == 'SMOTE-G':
        model = _smote_variants_v6_Gsmote.SMOTE(random_state=random_state)

    # W-smote
    elif method == 'SMOTE_ENN-W':
        model = _smote_variants_v2_Wsmote.SMOTE_ENN(random_state=random_state)
    elif method == 'SMOTE_TomekLinks-W':
        model = _smote_variants_v2_Wsmote.SMOTE_TomekLinks(
            random_state=random_state)
    elif method == 'SMOTE_RSB-W':
        model = _smote_variants_v2_Wsmote.SMOTE_RSB(random_state=random_state)
    elif method == 'SMOTE_IPF-W':
        model = _smote_variants_v2_Wsmote.SMOTE_IPF(random_state=random_state)
    elif method == 'SMOTE-W':
        model = _smote_variants_v2_Wsmote.SMOTE(random_state=random_state)

    # SW
    elif method == 'SMOTE_ENN-SW':
        model = _smote_variants_v3_SW.SMOTE_ENN(random_state=random_state)
    elif method == 'SMOTE_TomekLinks-SW':
        model = _smote_variants_v3_SW.SMOTE_TomekLinks(
            random_state=random_state)
    elif method == 'SMOTE_RSB-SW':
        model = _smote_variants_v3_SW.SMOTE_RSB(random_state=random_state)
    elif method == 'SMOTE_IPF-SW':
        model = _smote_variants_v3_SW.SMOTE_IPF(random_state=random_state)
    elif method == 'SMOTE-SW':
        model = _smote_variants_v3_SW.SMOTE(random_state=random_state)
        
    # v7 GB_smote
    elif method == 'SMOTE_ENN-GB':
        model = _smote_variants_v7_GBsmote.SMOTE_ENN(random_state=random_state)
    elif method == 'SMOTE_TomekLinks-GB':
        model = _smote_variants_v7_GBsmote.SMOTE_TomekLinks(random_state=random_state)
    elif method == 'SMOTE_RSB-GB':
        model = _smote_variants_v7_GBsmote.SMOTE_RSB(random_state=random_state)
    elif method == 'SMOTE_IPF-GB':
        model = _smote_variants_v7_GBsmote.SMOTE_IPF(random_state=random_state)
    elif method == 'SMOTE-GB':
        model = _smote_variants_v7_GBsmote.SMOTE(random_state=random_state)
    # --------------------------------------------------------------------------
    ...
    # Ohers----------------------------------------------------------------------
        ...
        model = _imblearn_v5_NanRd_v2.KMeansSMOTE(
            random_state=random_state, weight=weight_WNND, nans=nans)
        ...
        model = MAHAKIL()
        X_resampled, y_resampled = model.fit_sample(data_train[:, 1:], data_train[:, 0])
        y_resampled = y_resampled.reshape(y_resampled.shape[0], 1)
        data_train_resampled = np.hstack((y_resampled, X_resampled))
        return data_train_resampled
    elif method == 'SOMO':
        model = _smote_variants_v1_orignal.SOMO(random_state=random_state)
    elif method == 'PAO':
        model = PAO(gamma=0.1, lambd=0.0, random_state=42)
        X_resampled, y_resampled = model.sample(data_train[:, 1:], data_train[:, 0])
        y_resampled = y_resampled.reshape(y_resampled.shape[0], 1)
        data_train_resampled = np.hstack((y_resampled, X_resampled))
        return data_train_resampled
    elif method == 'SWIM':
        
        model = SwimMaha(sd=0.5)
        count = Counter(data_train[:, 0])
        X_resampled, y_resampled = model.mahaSampling(data_train[:, 1:], data_train[:, 0], numSamples=abs(count[0] - count[1]))
        y_resampled = y_resampled.reshape(y_resampled.shape[0], 1)
        data_train_resampled = np.hstack((y_resampled, X_resampled))
        return data_train_resampled
    ...
    #     return data_train_resampled
        ...
        X_resampled, y_resampled = MultiClassCCR(
            energy=0.25, cleaning_strategy='translate',
            selection_strategy='proportional', p_norm=1.0, method='sampling'
        ).fit_sample(data_train[:, 1:],  data_train[:, 0])
        y_resampled = np.asarray(y_resampled)
        y_resampled = y_resampled.reshape(y_resampled.shape[0], 1)
        data_train_resampled = np.hstack((y_resampled, X_resampled))
        return data_train_resampled
    elif method == 'MC_RBO':
        X_resampled, y_resampled = MultiClassRBO(method='sampling').fit_sample(data_train[:, 1:], data_train[:, 0])
        y_resampled = np.asarray(y_resampled)
        y_resampled = y_resampled.reshape(y_resampled.shape[0], 1)
        data_train_resampled = np.hstack((y_resampled, X_resampled))
        return data_train_resampled
        ...
        model = MDO()
        X_resampled, y_resampled =  model._fit_resample(np.copy(data_train[:, 1:]), np.copy(data_train[:, 0])) 
        y_resampled = np.asarray(y_resampled)
        y_resampled = y_resampled.reshape(y_resampled.shape[0], 1)
        data_train_resampled = np.hstack((y_resampled, X_resampled))
        return data_train_resampled
...
#   Post framework Oversampling
    if '-SW' in method:
        X_resampled, y_resampled = model.sample(RData[:, 1:], RData[:, 0],weight_SW,ntree)     
    elif '-Nan' in method:
        X_resampled, y_resampled = model.sample(data_train[:, 1:], data_train[:, 0], weight_WNND, nans)     
    else:X_resampled, y_resampled = model.sample(data_train[:, 1:], data_train[:, 0])
...
def __test_sampling():
    ...
    for data_name in data_names:
        ...
        print('{:20s}采样前：'.format(''), [counter[k]
