...
from sklearn import preprocessing  # For data preprocessing (e.g., normalization)

# Function to add labels to the dataset
def add_label(X, y):
    """
    Adds the label column to the feature matrix X.
    ...
    Returns:
    numpy.ndarray: Combined dataset with labels.
    """
    data = np.insert(X, X.shape[1], y, axis=1)
    return data

# Local Density-based Adaptive Sampling (LDAS) class
class LDAS:
    def __init__(self, k=5, k1=5, w=0.02):
        """
        Initializes the LDAS object with specified parameters.
        
        Parameters:
        k (int): Number of neighbors for density calculation.
        k1 (int): Number of neighbors for overlap removal.
        w (float): Weight parameter for setting the cutoff distance.
        """
        ...
        self.nnarray = []  # Stores nearest neighbors indices
        self.Synthetic = []  # Stores synthetic samples
        self.maj_index = []  # Stores indices of majority class samples
        self.min_index = []  # Stores indices of minority class samples

    def set_dc(self, N, dist):
        """
        Sets the cutoff distance (dc) based on the average distance of the K-th nearest neighbor.
    ...
    def cal_density(self, N, min_sample):
        """
        Calculates the density of each minority class sample.
        
        Parameters:
        N (int): Number of minority class samples.
        min_sample (numpy.ndarray): Minority class samples.
        
        Returns:
        numpy.ndarray: Normalized density values.
        """
    ...
    def remove_Overlap_Majority(self, X, y, indices, dist, density, N):
        """
        Removes overlapping majority class samples.
        ...
        dist (numpy.ndarray): Distance matrix.
        density (numpy.ndarray): Density values of minority class samples.
        N (int): Number of minority class samples.
        
        Returns:
        tuple: (X_removed, y_removed, need_num, p)
            X_removed (numpy.ndarray): Feature matrix after removing overlapping samples.
            y_removed (numpy.ndarray): Label array after removing overlapping samples.
            need_num (int): Number of synthetic samples needed.
            p (numpy.ndarray): Probability values for each minority class sample.
        """
        q = {}
        p = np.zeros(N)
    ...
    def cal_weight(self, a, b):
        """
        Calculates the weight for each minority class sample.
        
        Parameters:
        a (numpy.ndarray): Density values.
        b (numpy.ndarray): Border degree values.
        
        Returns:
        numpy.ndarray: Normalized weight values.
        """
        return self.normalize(a ** 2 + b ** 2)

    def cal_border_degree(self, p_list):
        """
        Calculates the border degree for each minority class sample.
        
        Parameters:
        p_list (numpy.ndarray): Probability values.
        
        Returns:
        list: Border degree values.
        """
        border_degree_list = []
        for p in p_list:
            if p == 1 or p == 0:
    ...
    def cal_num_to_gen(self, weight, G):
        """
        Calculates the number of synthetic samples to generate for each minority class sample.
        
        Parameters:
        weight (numpy.ndarray): Weight values.
        G (int): Total number of synthetic samples needed.
        
        Returns:
        numpy.ndarray: Number of synthetic samples to generate for each minority class sample.
        """
        sum = np.sum(weight)
    ...
    def populate(self, i, g, p):
        ...
        Parameters:
        i (int): Index of the minority class sample.
        g (numpy.ndarray): Number of synthetic samples to generate for each minority class sample.
        p (numpy.ndarray): Minority class samples.
        """
        N = len(p)
        if N < self.k + 1:
            range_k = N - 1
    ...
    def fit_sample(self, X, y):
        """
        Fits the LDAS model and generates synthetic samples.
        ...
        Returns:
        tuple: (X_new, y_new)
            X_new (numpy.ndarray): Feature matrix after oversampling.
            y_new (numpy.ndarray): Label array after oversampling.
        """
